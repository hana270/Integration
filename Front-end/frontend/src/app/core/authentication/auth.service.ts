...
export class AuthService {
  ...
  getProfileImageUrl(): Observable<string> {
    return this.getUserProfile().pipe(
      ...
      catchError((error) => {
        ...
        return of('assets/default-profile.jpg'); // Retourne l'image par défaut en cas d'erreur
      })
    );
  }
  ...
  registerInstaller(installerData: any): Observable<HttpResponse<any>> {
    return this.http
      .post<any>(`${this.apiURL}/register-installer`, installerData, {
        observe: 'response',
      })
      .pipe(
        tap((response) => {
          ...
          if (jwt) {
            ...
            console.error(
              'No JWT token found in installer registration response',
              response
            );
        ...
        catchError((error) => {
          ...
          Swal.fire(
            'Erreur',
            error.error?.message || "Échec de l'inscription de l'installateur",
            'error'
          );
    ...
    return this.http
      ...
      .pipe(
        tap((response) => {
          ...
          let jwt =
            response.body?.token || response.headers.get('Authorization');
